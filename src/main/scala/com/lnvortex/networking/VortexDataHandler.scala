...
class VortexDataHandler(dlcWalletApi: DLCWalletApi, connectionHandler: ActorRef)
    extends Actor
    with ActorLogging {
  ...
  override def receive: Receive = LoggingReceive {
    ...
    case VortexConnectionHandler.WriteFailed(_) =>
...
object DLCDataHandler {
  ...
  def props(dlcWalletApi: DLCWalletApi, connectionHandler: ActorRef): Props =
    Props(new VortexDataHandler(dlcWalletApi, connectionHandler))
}
